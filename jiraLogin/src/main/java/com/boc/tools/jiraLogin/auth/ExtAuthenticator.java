package com.boc.tools.jiraLogin.auth;
...
import com.boc.tools.jiraLogin.util.UimUtil;
...
public class ExtAuthenticator extends DefaultAuthenticator {
    private static final Logger log = Logger.getLogger(ExtAuthenticator.class);
    ...
    protected boolean authenticate(final Principal user, final String password) throws AuthenticatorException {
        if (null == getUser(user.getName().toLowerCase())) {
            return false;
        }
        if (UimUtil.isExtUser(user.getName().toLowerCase()))
        {
            return UimUtil.extVerify(user.getName(), password);
        }
        try {
            ...
            log.debug("authentication failed: '" + user.getName(), e);
            return false;
